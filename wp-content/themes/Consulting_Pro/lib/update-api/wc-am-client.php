	...
	class WC_AM_Client {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
			...
			if ( is_admin() ) {
				...
				$this->wc_am_plugin_name       = $this->plugin_or_theme == 'plugin' ? untrailingslashit( plugin_basename( $this->file ) ) : get_template(); // same as plugin slug. if a theme use a theme name like 'twentyeleven'
		...
		public function license_key_deactivation() {
			...
			$api_key           = get_option( $this->wc_am_api_key_key );
		...
		public function inactive_notice() { ?>
            ...
            <div class="notice notice-error tut-notice-api">
		...
		public function wc_am_api_key_field() {
			...
			$tut_api_key = get_option( $this->wc_am_api_key_key );

			if ( ! empty( $tut_api_key ) ) {
				echo "<input id='api_key' name='" . esc_attr( $this->data_key ) . "[" . esc_attr( $this->wc_am_api_key_key ) . "]' size='25' type='text' value='" . esc_attr( $tut_api_key ) . "' />";
		...
		public function validate_options( $input ) {
			...
			$current_api_key                     = get_option( $this->wc_am_api_key_key );
...
//					if ( $activate_results[ 'success' ] === true && $activate_results[ 'activated' ] === true ) {
					if ( $activate_results[ 'success' ] === true ) {
						...
						update_option( $this->wc_am_api_key_key, $api_key );
						update_option( $this->wc_am_activated_key, 'Activated' );
						update_option( $this->wc_am_deactivate_checkbox_key, 'off' );
					}
...
//					if ( isset( $activate_results[ 'data' ][ 'error_code' ] ) && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
					if ( isset( $activate_results[ 'data' ][ 'error_code' ] ) ) {
						add_settings_error( 'wc_am_client_error_text', 'wc_am_client_error', "{$activate_results['data']['error']}", 'error' );
						update_option( $this->data[ $this->wc_am_activated_key ], 'Deactivated' );
					}
		...
		public function wc_am_license_key_deactivation( $input ) {
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $tut_api_key != '' ) {
...
//				if ( $activate_results[ 'success' ] === true && $activate_results[ 'deactivated' ] === true ) {
				if ( $activate_results[ 'success' ] === true ) {
					...
					if ( ! empty( $this->wc_am_activated_key ) ) {
						...
						update_option( $this->wc_am_api_key_key, '' );
						update_option( $this->wc_am_activated_key, 'Deactivated' );
						update_option( $this->data_key, '' );
		...
		public function activate( $args ) {
			...
			$request    = wp_safe_remote_post( $target_url, array(
								'timeout' => 15
							)
						);
