...
class thinkup_builder_imagetheme extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 'classname' => 'thinkup_builder_imagetheme', 'description' => __( 'Add an image from the media library.', 'consulting' ) );
		$control_ops = array( 'id_base' => 'thinkup_builder_imagetheme' );
		parent::__construct('thinkup_builder_imagetheme', __('Image', 'consulting'), $widget_ops, $control_ops);
	...
	function admin_setup() {
		...
		wp_localize_script( 'thinkup-builder-image', 'ThinkupBuilderImage', array(
			'frame_title' => __( 'Select an Image', 'consulting' ),
			'button_title' => __( 'Insert Into Widget', 'consulting' ),
		) );
	}
...
//		$instance       = wp_parse_args( (array) $instance, self::get_defaults() );
		...
		echo '<p style="margin-bottom: 20px;"><label for="' . $this->get_field_id('title') . '" style="display: inline-block;width: 100px;" >' . __('Widget Title', 'consulting') . ':</label>',
		...
		echo '<div class="uploader">',
			 '<input type="submit" class="button" name="' . $this->get_field_name('uploader_button') . '" id="' . $this->get_field_id('uploader_button') . '" value="' . __('Select an Image', 'consulting') . '" onclick="imageWidget.uploader( &#39;' . $id_prefix . '&#39;, &#39;' . $id_prefix . '&#39; ); return false;" />',
			 ...
			 '</div>',
			 '<input type="hidden" id="' . $this->get_field_id('attachment_id') .'" name="' . $this->get_field_name('attachment_id') .'" value="' . abs((int) $instance['attachment_id']) .'" />',
		...
		echo '<div id="' . $this->get_field_id('custom_size_selector') . '" class="image-image-size" >',
			 '<p style="margin-bottom: 20px;"><label for="' . $this->get_field_id('size') . '" style="display: inline-block;width: 150px;">' . __('Size', 'consulting') . ':</label>',
			 '<select name="' . $this->get_field_name('size') . '" id="' . $this->get_field_id('size') . '" onChange="imageWidget.toggleSizes( &#39;' . $id_prefix . '&#39;, &#39;' . $id_prefix . '&#39; );" style="display: inline-block;width: 200px;margin: 0;">';
					...
					$possible_sizes = apply_filters( 'image_size_names_choose', array(
						'full'      => __('Full Size', 'consulting'),
						'thumbnail' => __('Thumbnail', 'consulting'),
						'medium'    => __('Medium', 'consulting'),
						'large'     => __('Large', 'consulting'),
					) );
	//				$possible_sizes[self::CUSTOM_IMAGE_SIZE_SLUG] = __('Custom', 'consulting');
		...
		echo '<p><label for="' . $this->get_field_id('link') . '" style="display: inline-block;width: 150px;" >' . __('Link', 'consulting') . ':</label>',
	...
	function widget( $args, $instance ) {
		...
		if ( !empty( $instance['imageurl'] ) || !empty( $instance['attachment_id'] ) ) {
			...
			if ( $lightbox == 'on' ) {
				...
				echo '<p' . $align .'><img src="' . $image_img[0] . '" alt="' . esc_attr( $text_alt ) . '" / ></p>';					
			} else if ( $overlay_enable == 'on' ) {
				echo '<div' . $align . ' class="sc-image sc-carousel">';
				echo '<div class="entry-header">';
				echo '<a href="' . $link . '"><img src="' . $image_img[0] . '" alt="' . esc_attr( $text_alt ) . '" /></a>';
	...
	private function get_image_aspect_ratio( $instance ) {
		if ( !empty( $instance['aspect_ratio'] ) ) {
			return abs((int) $instance['aspect_ratio'] );
		} else {
