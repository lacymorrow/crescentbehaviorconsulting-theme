...
global $wp_embed;
	 ...
	 $_thinkup_meta_featuredmediamain = get_post_meta( $recent["ID"], '_thinkup_meta_featuredmediamain', true );
		...
		if ( ! empty( $_thinkup_meta_featuredmedia ) and $_thinkup_meta_featuredmediamain == 'option1' ) {
			...
			if ( $_thinkup_meta_featuredmediatype == 'option1' ) {
				...
				$thinkup_input_featured = '<iframe src="' . esc_url( $thinkup_input_featured ) . '"></iframe>';
			}
		} else {
			$thinkup_input_featured = '<img src="' . esc_url( $post_img[0] ) . '" />';
		}
	...
	if ( $link_icon !== 'off' ) {
		$link_input = '<a class="hover-link" href="' . esc_url( get_permalink( $recent["ID"] ) ) . '"></a>';
	}

	// Set lightbox icon variable if user wants it to show
	if ( $lightbox_icon !== 'off' ) {
		$lightbox_input = '<a class="hover-zoom prettyPhoto" href="' . esc_url( $thinkup_input_link ) . '"></a>';
	}
	...
	if ( ! empty( $post_id ) ) {
		...
		if ( $title == 'on' or $details == 'on' ) {
			...
			$output_content .= '<div class="port-details' . $portfolio_styleclass . '">';

			if ( $title == 'on' ) {
				$output_content .= '<h4 class="port-title"><a href="' . esc_url( get_permalink( $recent["ID"] ) ) . '">' . get_the_title( $recent["ID"] ) . '</a></h4>'; 
			}
		...
		if ( $link_icon !== 'off' or $lightbox_icon !== 'off' or ! empty( $output_content2 ) ) {
