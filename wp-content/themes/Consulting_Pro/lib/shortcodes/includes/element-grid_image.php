	...
	?><?php foreach((array) $groups['image'] as $increment=>$context){ ?><?php
		...
		if ( ! empty( $image ) ) {
			...
			if ( ! empty( $animate ) and $animate !== 'none' ) {
				...
				$animate_start = '<div class="animated start-' . $animate . '" title="' . esc_attr( $delay ) . '">';
