...
echo '<div class="sc-carousel carousel-testimonial ' . $style . '" data-show="' . $show . '" data-scroll="' . $scroll . '" data-speed="' . $speed . '" data-effect="' . $effect . '">';
	...
	foreach( $recent_posts as $recent ){
		...
		if ( $links == 'off' ) {
			...
			$link_start = '<a href="' . esc_url( get_permalink( $recent["ID"] ) ) . '">';
			$link_end   = '</a>';
		}
		...
		if ( empty( $style ) or $style == 'style1' ) {
			...
			$post_img = wp_get_attachment_image_src($post_id, 'sc-testimonial', true);
			...
			echo $link_start . '<img src="' . esc_url( $post_img[0] ) . '" />' . $link_end;
			echo '</div>';
	...
	if ( empty( $style ) or $style == 'style1' ) {
		...
		foreach( $recent_posts as $recent ){
			...
			echo '<a href="#' . $instanceID . '-' . $i . '" class="' . $selected . '"><img src="' . esc_url( $post_img[0] ) . '" /></a>';
			$i++;
		}
		echo '</div>';
