...
global $thinkup_header_float;
global $thinkup_header_logooverflow;
global $thinkup_header_locationswitch;

// Set variables to avoid php non-object notice error
// Set variables to avoid php non-object notice error
$_thinkup_meta_headerstyle        = NULL;
$_thinkup_meta_headerfloat        = NULL;
$_thinkup_meta_headerlogooverflow = NULL;
$_thinkup_meta_headerlocation     = NULL;

// Assign meta data variable
if ( ! empty( $post->ID ) ) {
	$_thinkup_meta_headerstyle        = get_post_meta( $post->ID, '_thinkup_meta_headerstyle', true );
	$_thinkup_meta_headerfloat        = get_post_meta( $post->ID, '_thinkup_meta_headerfloat', true );
	$_thinkup_meta_headerlogooverflow = get_post_meta( $post->ID, '_thinkup_meta_headerlogooverflow', true );
	...
	if ( empty( $_thinkup_meta_headerstyle ) or $_thinkup_meta_headerstyle == 'option1' ) {
		...
		$classes[] = 'header-style1';
			...
			// Check whether floating header is enabled
			if ( $_thinkup_meta_headerfloat == 'on' ) {
				$classes[] = 'header-float';
			}

			// Check whether header logo should overflow container
			if ( $_thinkup_meta_headerlogooverflow == 'on' ) {
				$classes[] = 'header-logooverflow';
			}
...
$class_header1_searchshow  = NULL;
$class_header2_searchshow  = NULL;
$class_header2_searchstart = NULL;
$class_header2_searchend   = NULL;
...
if ( empty( $_thinkup_meta_headerstyle ) or $_thinkup_meta_headerstyle == 'option1' ) {
	if ( $thinkup_header_styleswitch == 'option2' and $thinkup_header_socialswitchmain == '1' ) {
		$class_header1_searchshow = '1';
	}
} else if ( $_thinkup_meta_headerstyle == 'option3' ) {
	$class_header1_searchshow = '1';
}

// Assign variables to add #header social media icons next to logo for header style 3
if ( empty( $_thinkup_meta_headerstyle ) or $_thinkup_meta_headerstyle == 'option1' ) {
	if ( $thinkup_header_styleswitch == 'option3' and $thinkup_header_searchswitch == '1' ) {
		$class_header2_searchstart = '<div id="header-container">';
		$class_header2_searchend   = '</div>';
		$class_header2_searchshow  = '1';
	}
} else if ( $_thinkup_meta_headerstyle == 'option4' ) {
	$class_header2_searchstart = '<div id="header-container">';
	$class_header2_searchend   = '</div>';
...
?>
		...
		<div id="header-core">
			...
			<?php echo $class_header2_searchend; ?>
...
$output_stickylogo = NULL;
	...
	if ( $thinkup_header_stickyswitch == '1' ) {
		...
		if ( ! empty( $thinkup_general_logolinksticky ) ) {
			$output_stickylogo = '<a rel="home" href="' . esc_url( home_url( '/' ) ) . '"><img src="' . esc_url( $thinkup_general_logolinksticky ) . '" alt="' . esc_attr__( 'Logo', 'consulting' ) . '"></a>';
		} else {
...
$j = 0;
	...
	if ( $thinkup_header_socialswitchpre == '1' ) {
		...
		$social_links = array(
			...
			array( 'social' => __( 'Instagram', 'consulting' ), 'icon' => 'instagram',    'toggle' => $thinkup_header_instagramswitch, 'link' => $thinkup_header_instagramlink ),
			array( 'social' => __( 'Tumblr', 'consulting' ),    'icon' => 'tumblr',       'toggle' => $thinkup_header_tumblrswitch,    'link' => $thinkup_header_tumblrlink ),
			...
			array( 'social' => __( 'Pinterest', 'consulting' ), 'icon' => 'pinterest',    'toggle' => $thinkup_header_pinterestswitch, 'link' => $thinkup_header_pinterestlink ),
			array( 'social' => __( 'Xing', 'consulting' ),      'icon' => 'xing',         'toggle' => $thinkup_header_xingswitch,      'link' => $thinkup_header_xinglink ),
			array( 'social' => __( 'PayPal', 'consulting' ),    'icon' => 'paypal',       'toggle' => $thinkup_header_paypalswitch,    'link' => $thinkup_header_paypallink ),
			array( 'social' => __( 'Vimeo', 'consulting' ),     'icon' => 'vimeo-square', 'toggle' => $thinkup_header_vimeoswitch,     'link' => $thinkup_header_vimeolink ),
			...
			array( 'social' => __( 'Email', 'consulting' ),     'icon' => 'envelope',     'toggle' => $thinkup_header_emailswitch,     'link' => $thinkup_header_emaillink ),
		);
