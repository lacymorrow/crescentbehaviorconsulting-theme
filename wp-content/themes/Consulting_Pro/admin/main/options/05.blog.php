...
$_thinkup_meta_blogstyle2layout = get_post_meta( $thinkup_blog_pageid, '_thinkup_meta_blogstyle2layout', true );
	...
	if ( get_page_template_slug( $thinkup_blog_pageid ) !== 'template-blog.php' ) {
		if ( $thinkup_blog_style == 'option1' ) {
			...
			if ( $thinkup_blog_style2layout == 'option1' ) {
				$output = ' column-1';
		...
		if ( empty( $_thinkup_meta_blogstyle ) or $_thinkup_meta_blogstyle == 'option1' ) {
			...
			if ( $_thinkup_meta_blogstyle2layout == 'option1' ) {
				$output = ' column-1';
...
$_thinkup_meta_blogstyle1layout = get_post_meta( $thinkup_blog_pageid, '_thinkup_meta_blogstyle1layout', true );
	...
	if ( has_post_thumbnail( $post->ID ) and $thinkup_blog_postswitch !== 'option2' ) {
		if ( get_page_template_slug( $thinkup_blog_pageid ) !== 'template-blog.php' ) {
			if ( $thinkup_blog_style == 'option1' and $thinkup_blog_style1layout !== 'option2' ) {
				echo ' two_fifth';
			}
		} else if ( get_page_template_slug( $thinkup_blog_pageid ) == 'template-blog.php' ) {
			if ( empty( $_thinkup_meta_blogstyle ) or $_thinkup_meta_blogstyle == 'option1' ) {
...
global $thinkup_blog_title;
	...
	if ( $thinkup_blog_title !== '1' ) {
		echo	'<h2 class="blog-title">',
				'<a href="' . esc_url( get_permalink() ) . '" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'consulting' ), the_title_attribute( 'echo=0' ) ) ) . '">' . get_the_title() . '</a>',
				'</h2>';
	}
}
...
// Set variables to avoid php non-object notice error
$_thinkup_meta_featuredmedia   = NULL;
$_thinkup_meta_featuredgallery = NULL;
...
if ( ! empty( $post->ID ) ) {
	...
	// Set default size for blog thumbnail
	if( empty( $size ) ) {
		$size = 'column3-2/3';
	}
	...
	if ( get_post_format() == 'video' and ! empty( $_thinkup_meta_featuredmedia ) ) {
		...
		foreach ( $_thinkup_meta_featuredgallery['image'] as $slide => $list) {
			...
			$media .= '<li>';
			$media .= '<img src="' . esc_url( get_template_directory_uri() ) . '/images/transparent.png" style="' . esc_attr( $slide_image ) . '" alt="' . esc_attr__( 'Image', 'consulting' ) . '" />';
			$media .= '</li>';
...
if ( ! empty( $thinkup_blog_pageid ) ) {
	...
	if( empty( $thinkup_translate_blogreadmore ) ) {
		$thinkup_translate_blogreadmore = __( 'Read More', 'consulting');
	}

	$link = '&hellip;<p class="more-link"><a href="' . esc_url( get_permalink($post->ID) ) . '" class="' . $class_button . '">' . esc_html( $thinkup_translate_blogreadmore ) . '</a></p>';
...
if ( ! empty( $post->ID ) ) {
	...
	if ( get_post_format() == 'image' ) {
		...
		foreach ( $_thinkup_meta_featuredgallery['image'] as $slide => $list) {
			...
			$output .= '<li>';
			$output .= '<img src="' . esc_url( get_template_directory_uri() ) . '/images/transparent.png" style="' . esc_attr( $slide_image ) . '" alt="' . esc_attr__( 'Image', 'consulting' ) . '" />';
			$output .= '</li>';
