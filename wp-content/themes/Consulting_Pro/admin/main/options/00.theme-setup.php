...
function thinkup_title_select() {
	...
	if ( is_page() ) {
		...
		printf( '<span>' . __( 'Blog Post Image: ', 'consulting' ) . '</span>' . '%s', esc_html( get_the_title( $post->post_parent ) ) );
	} else if ( is_single() ) {
		printf( '<span>%s</span>', html_entity_decode( esc_html( get_the_title() ) ) );
	} else if ( is_search() ) {
		printf( '<span>' . __( 'Search Results: ', 'consulting' ) . '</span>' . '%s', esc_html( get_search_query() ) );
	} else if ( is_404() ) {
		printf( '<span>' . __( 'Page Not Found', 'consulting' ) . '</span>' );
	} else if ( is_category() ) {
		printf( '<span>' . __( 'Category Archives: ', 'consulting' ) . '</span>' . '%s', single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		printf( '<span>' . __( 'Tag Archives: ', 'consulting' ) . '</span>' . '%s', single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		the_post();
		printf( '<span>' . __( 'Author Archives: ', 'consulting' ) . '</span>' . '%s', get_the_author() );
		rewind_posts();
	} elseif ( is_day() ) {
		printf( '<span>' . __( 'Daily Archives: ', 'consulting' ) . '</span>' . '%s', get_the_date() );
	} elseif ( is_month() ) {
		printf( '<span>' . __( 'Monthly Archives: ', 'consulting' ) . '</span>' . '%s', get_the_date( 'F Y' ) );
	} elseif ( is_year() ) {
		printf( '<span>' . __( 'Yearly Archives: ', 'consulting' ) . '</span>' . '%s', get_the_date( 'Y' ) );
	} elseif ( is_post_type_archive( 'portfolio' ) ) {
		printf( '<span>' . __( 'Portfolio', 'consulting' ) . '</span>' );
	} elseif ( is_post_type_archive( 'client' ) ) {
		printf( '<span>' . __( 'Our Clients', 'consulting' ) . '</span>' );
	} elseif ( is_post_type_archive( 'team' ) ) {
		printf( '<span>' . __( 'Our Team', 'consulting' ) . '</span>' );
	} elseif ( is_post_type_archive( 'testimonial' ) ) {
		printf( '<span>' . __( 'Testimonials', 'consulting' ) . '</span>' );
		...
		printf( '<span>' . __( 'Blog', 'consulting' ) . '</span>' );
	} else {
		printf( '<span>%s</span>', html_entity_decode( esc_html( get_the_title() ) ) );
	}
}
...
global $thinkup_general_breadcrumbdelimeter;
	...
	if ( ! is_front_page() ) {
		...
		if ( is_single() ) {
			...
			$output .= '<span class="breadcrumbs-cat">' . __( 'Archive Category: ', 'consulting' ) . '</span>' . esc_html( single_cat_title("", false) );
		} elseif ( is_tag() ) {
			$output .= '<span class="breadcrumbs-tag">' . __( 'Posts Tagged: ', 'consulting' ) . '</span>' . single_tag_title("", false);
		} elseif ( is_day()) {
			...
			$output .= __( 'Archived Article(s) by Author: ', 'consulting' ) . esc_html( $user_info->display_name );
		} elseif ( is_404() ) {
			$output .= __( 'Error 404 - Not Found.', 'consulting' );
		} elseif ( is_post_type_archive( 'portfolio' )	) {
			$output .= __( 'Portfolio', 'consulting' );
		} elseif ( is_post_type_archive( 'client' )	) {
			$output .= __( 'Clients', 'consulting' );
		} elseif ( is_post_type_archive( 'team' )	) {
			$output .= __( 'Team', 'consulting' );
		} elseif ( is_post_type_archive( 'testimonial' )	) {
			$output .= __( 'Testimonials', 'consulting' );
		} elseif ( is_post_type_archive( 'product' ) and function_exists( 'thinkup_woo_titleshop_archive' ) ) {
...
if ( ! function_exists( 'thinkup_input_showimagesizes' ) ) {
	function thinkup_input_showimagesizes($sizes) {
		...
		// 1 Column Layout
		$sizes['column1-1/2'] = __( 'Full - 1:2', 'consulting' );
		$sizes['column1-1/3'] = __( 'Full - 1:3', 'consulting' );
		$sizes['column1-1/4'] = __( 'Full - 1:4', 'consulting' );
		$sizes['column1-2/5'] = __( 'Full - 2:5', 'consulting' );

		// 2 Column Layout
		$sizes['column2-1/1'] = __( 'Half - 1:1', 'consulting' );
		$sizes['column2-1/2'] = __( 'Half - 1:2', 'consulting' );
		$sizes['column2-2/3'] = __( 'Half - 2:3', 'consulting' );
		$sizes['column2-3/5'] = __( 'Half - 3:5', 'consulting' );

		// 3 Column Layout
		$sizes['column3-1/1'] = __( 'Third - 1:1', 'consulting' );
		$sizes['column3-2/5'] = __( 'Third - 2:5', 'consulting' );
		$sizes['column3-2/3'] = __( 'Third - 2:3', 'consulting' );
		$sizes['column3-3/4'] = __( 'Third - 3:4', 'consulting' );

		// 4 Column Layout
		$sizes['column4-1/1'] = __( 'Quarter - 1:1', 'consulting' );
		$sizes['column4-2/3'] = __( 'Quarter - 2:3', 'consulting' );
		$sizes['column4-3/4'] = __( 'Quarter - 3:4', 'consulting' );
...
function thinkup_input_commentspopuplink( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
    ...
    if ( false === $zero ) $zero = __( 'No Comments','consulting' );
    if ( false === $one ) $one = __( '1 Comment','consulting' );
    if ( false === $more ) $more = __( '% Comments','consulting' );
    if ( false === $none ) $none = __( 'Comments Off','consulting' );
    ...
    if ( post_password_required() ) {
        $str = __('Enter your password to view comments.','consulting');
        return $str;
    }
    ...
    $str .= ' title="' . esc_attr( sprintf( __('Comment on %s','consulting'), $title ) ) . '">';
...
function thinkup_comments_returnstring( $zero = false, $one = false, $more = false, $deprecated = '' ) {
    ...
    if ( $number > 1 )
        $output = str_replace('%', number_format_i18n($number), ( false === $more ) ? __( '% Comments', 'consulting' ) : $more);
    elseif ( $number == 0 )
        $output = ( false === $zero ) ? __( 'No Comments', 'consulting' ) : $zero;
    else // must be one
        $output = ( false === $one ) ? __( '1 Comment', 'consulting' ) : $one;
...
function thinkup_posts_columns($defaults){
    $defaults['riv_post_thumbs'] = __( 'Thumbs', 'consulting' );
    return $defaults;
}
