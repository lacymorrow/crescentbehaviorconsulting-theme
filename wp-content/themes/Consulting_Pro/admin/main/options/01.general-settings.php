...
//Output header above slider - Consulting specific
...
//Output header below slider - Consulting specific
...
function thinkup_custom_introbackground() {
global $thinkup_general_introswitch;
global $thinkup_general_introbackgroundcolor;
global $thinkup_general_introbackgroundimage;

global $post;

// Set variables to avoid php non-object notice error
$_thinkup_meta_intro                = NULL;
$_thinkup_meta_introbackgroundcolor = NULL;
$_thinkup_meta_introbackgroundimage = NULL;
$output                             = NULL;

// Assign meta data variable (check that it's not an archive / search page)
if ( ! empty( $post->ID ) and thinkup_check_currentpage() == get_permalink() ) {
	$_thinkup_meta_intro                = get_post_meta( $post->ID, '_thinkup_meta_intro', true );
	$_thinkup_meta_introbackgroundcolor = get_post_meta( $post->ID, '_thinkup_meta_introbackgroundcolor', true );
	$_thinkup_meta_introbackgroundimage = get_post_meta( $post->ID, '_thinkup_meta_introbackgroundimage', true );
}

	if( empty( $_thinkup_meta_intro ) or $_thinkup_meta_intro == 'option1' ) {
		if ( empty( $thinkup_general_introswitch ) or $thinkup_general_introswitch == '1' ) {

			if( ! empty( $thinkup_general_introbackgroundimage ) ) {
				$output .= '#intro {';
				$output .= 'background: url(' . esc_url( $thinkup_general_introbackgroundimage ) . ');';
				$output .= 'background-size: cover;';
				$output .= 'background-position: center;';
				$output .= '}';
			} else if ( ! empty( $thinkup_general_introbackgroundcolor ) ) {
				$output .= '#intro {';
		...
		if( ! empty( $_thinkup_meta_introbackgroundimage ) ) {
			$output .= '#intro {';
			$output .= 'background: url(' . esc_url( $_thinkup_meta_introbackgroundimage ) . ') !important;';
			$output .= 'background-size: cover !important;';
			$output .= 'background-position: center !important;';
			$output .= '}';
		} else if ( ! empty( $_thinkup_meta_introbackgroundcolor ) ) {
			$output .= '#intro {';
			$output .= 'background: ' . esc_html( $_thinkup_meta_introbackgroundcolor ) . ' !important;';
			$output .= '}';
		}
