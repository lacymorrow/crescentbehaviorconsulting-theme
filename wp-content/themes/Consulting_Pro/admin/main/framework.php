...
function thinkup_var_cookie( $variable ) {
	...
	if ( empty( $thinkup_get_reduxvariables ) ) {
		...
		return isset($thinkup_get_reduxvariables[$variable]) ? $thinkup_get_reduxvariables[ $variable ] : null;
	}
}
endif;
                    ...
                    if ( isset ( $field['required'][0] ) ) {
                        if ( ! is_array( $field['required'][0] ) && count( $field['required'] ) == 3 ) {
                            ...
                            foreach ( $field['required'] as $required ) {
                                if ( ! is_array( $required[0] ) && count( $required ) == 3 && isset($GLOBALS[ $this->args['global_variable'] ][ $required[0] ]) ) {
